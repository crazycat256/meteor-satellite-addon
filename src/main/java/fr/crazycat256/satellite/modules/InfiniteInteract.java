...
public class InfiniteInteract extends Module {
    ...
    private void onAttack(AttackEntityEvent event) {
        ...
        if (notIfSneaking.get() && mc.player.isShiftKeyDown()) return;
    ...
    private void onInteractBlock(InteractBlockEvent event) {
        ...
        Vec3 blockPos = event.result.getLocation();

        if (blockPos.distanceTo(mc.player.position()) <= 5) return;
    ...
    private void onInteractEntity(InteractEntityEvent event) {
        if (mc.player == null || mc.gameMode == null || !interactEntities.get()) return;
        if (notIfSneaking.get() && mc.player.isShiftKeyDown()) return;
    ...
    private void onStartBreakingBlock(StartBreakingBlockEvent event) {
        ...
        Vec3 blockPos = new Vec3(event.blockPos.getX(), event.blockPos.getY(), event.blockPos.getZ());

        if (blockPos.distanceTo(mc.player.position()) <= 5) return;

        Vec3 playerPos = mc.player.position();

        Vec3 tempPos = tpTo(blockPos);

        mc.player.connection.send(new ServerboundPlayerActionPacket(ServerboundPlayerActionPacket.Action.START_DESTROY_BLOCK, event.blockPos, event.direction));
    ...
    private void onBreakBlock(BreakBlockEvent event) {
        ...
        mc.player.connection.send(new ServerboundPlayerActionPacket(ServerboundPlayerActionPacket.Action.STOP_DESTROY_BLOCK, event.blockPos, Direction.UP));
    ...
    private Vec3 tpTo(Vec3 tpPos) {
        Vec3 playerPos = mc.player.position();

        ArrayList<Vec3> path = findTPPath(playerPos, tpPos, 20, 3, 8);
    ...
    private void tpBack(Vec3 tempPos, Vec3 initialPos) {
        ArrayList<Vec3> path = findTPPath(tempPos, initialPos, 10, 0, 9);
        if (path == null) return;
        for (int i = 0; i < 20 - path.size(); i++) {
            mc.player.connection.send(new ServerboundMovePlayerPacket.StatusOnly(true, mc.player.horizontalCollision));
        }
